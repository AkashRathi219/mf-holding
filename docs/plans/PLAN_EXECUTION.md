...
CAS PDF fixture + production URL. **SEED1/SEED2 (done/doing)** — default demo
dataset: client "Client 1" with "Default Portfolio" built from BOTH CAS
reference files (holdings JSON = units+weights structure; transactions extract
= per-ISIN amfi_code/txn-count enrichment), seeded idempotently into
superadmin + demo accounts via `scripts/seed_default_portfolios.py` and the
`POST /api/seed-samples` endpoint; fixed the model-id lookup bug that left
early portfolios with `model_portfolio_id=NULL` (self-heals on reseed).
**6 Website** — 17 TODOs / 39 `YOURSITE.com` /
...
promotion evidence comes from loop L3 once S4 lands. **ANAF1 (done)** —
since-inception CAGR returns honest null when the NAV span is < 90 days
(`MIN_CAGR_WINDOW_DAYS`); previously a 5-point history annualized into
nonsense (e.g. -17.23%) in the compare metric table. **8 Debt** — FK cascades
